...
V1 lifecycle reporting also runs in the pane-local TUI. The selected root stays working while it or any descendant is busy or retrying, and blocked while that session family has a pending permission or question. Attaching to an existing session loads its current activity and pending requests. Browsing a child keeps the root session identity used for restore. V1 Mini and headless clients do not run this TUI plugin and do not provide these lifecycle reports; the server entrypoint is inactive.

When upgrading a shared V1 server from an older integration, restart that server as well as its attached TUIs to unload the old server lifecycle reporter.

The plugin reports lifecycle state and session identity while OpenCode runs inside a Herdr pane. After the TUI selects a session, Herdr can use the reported root session id to resume the pane with `opencode --session <id>`. Native screen manifest detection remains available when the plugin is not installed.
