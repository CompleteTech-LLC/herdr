...
V1 のライフサイクル報告もペイン内の TUI で実行されます。選択されたルートまたはその子孫が実行中・再試行中なら working、そのセッション群に未処理の権限要求や質問があれば blocked になります。既存のセッションに接続すると、現在の動作状況と未処理の要求を読み込みます。子セッションを表示しても、復元用のルートセッション ID は維持されます。V1 Mini とヘッドレスクライアントはこの TUI プラグインを実行せず、ライフサイクル報告を提供しません。サーバー側のエントリポイントは報告を行いません。

共有 V1 サーバーで古い連携から更新する場合は、接続中の TUI に加えてサーバーも再起動し、古いサーバー側のライフサイクル報告を停止してください。

このプラグインは、OpenCode が Herdr のペイン内で動いている間、ライフサイクル状態とセッション識別を報告します。TUI でセッションを選択した後、Herdr は報告されたルートセッション ID を使って `opencode --session <id>` でペインを復元できます。プラグインがインストールされていないときは、スクリーンマニフェスト検出が引き続き利用できます。
