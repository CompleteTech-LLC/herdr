...
V1 生命周期上报也在窗格本地的 TUI 中运行。只要所选根会话或其任何后代仍在忙碌或重试，状态就保持 working；该会话组有待处理的权限请求或问题时，状态为 blocked。连接已有会话时会读取当前活动和待处理请求。查看子会话仍保留用于恢复的根会话标识。V1 Mini 和无界面客户端不运行此 TUI 插件，也不提供这些生命周期上报；服务端入口不再上报状态。

共享 V1 服务端从旧版集成升级时，请同时重启服务端及其已连接的 TUI，以停止旧版服务端的生命周期上报。

该插件在 OpenCode 运行于 Herdr 窗格内时上报生命周期状态和会话身份。TUI 选中会话后，Herdr 可以用上报的根会话 ID 通过 `opencode --session <id>` 恢复该窗格。插件未安装时，屏幕清单检测仍然可用。
